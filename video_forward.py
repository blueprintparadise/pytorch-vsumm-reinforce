...
#训练
python3 main.py -d datasets/eccv16_dataset_summe_google_pool5.h5 -s datasets/summe_splits.json -m summe --gpu 0 --save-dir log/summe-split0 --split-id 0 --verbose
...
def video2summary(h5_dir,frm_dir,output_dir):
    ...
    for idx1 in range(len(list(h5_res.keys()))):
        ...
        vid_writer = cv2.VideoWriter(
            ...
            osp.join(output_dir,'train', "summary.mp4"),
            cv2.VideoWriter_fourcc(*'MP4V'),
            fps,#args.fps,
            (args.width, args.height),
        )
        frm2video(frm_dir, summary, vid_writer,video_name)
        vid_writer.release()
    h5_res.close()
...
if __name__ == '__main__':
    if args.makedatasets:
        ...
        output = args.dataset
        gen = Generate_Dataset(video,output,args.frm_dir)
